...
export class LoginComponent {
  ...
  constructor(private renderer: Renderer2, private authService: AuthService, private router: Router, private emailService: EmailService) { }
    ...
    if (this.isSignInSelected) {
      ...
      this.authService.register(username, email, password).subscribe(
        response => {
          ...
          this.emailService.sendWelcomeEmail(username, email).then(
            response => console.log('Correo electrónico enviado', response),
            error => console.error('Error al enviar correo electrónico', error)
          );
          this.authService.login(email, password).subscribe(
            response => {
              console.log('Login success:', response);
